...
const search: any = ref(cityName.value);
const input = ref("");
const background = ref("");
// const { data: city, error } = useFetch(
//   () =>
//     `https://api.openweathermap.org/data/2.5/weather?q=${search.value}&units=metric&appid=9359c1e4d5e4fad8abf98a108d5643e0`
// );
    ...
    try {
      response = await $fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${search.value}`,
        {
          params: {
            units: "metric",
            appid: "9359c1e4d5e4fad8abf98a108d5643e0",
          },
        }
      );
...
<template>
  <div class="h-screen relative overflow-hidden" v-if="city">
    <img :src="background" />
    <div class="absolute w-full h-full top-0 overlay" />
    <div class="absolute w-full h-full top-0 p-48">
      <div class="flex justify-between">
        <div>
          <h1 class="text-7xl text-white mb-7">{{ city.name }}</h1>
